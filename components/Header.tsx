...
export default function Header({ locale }: { locale: Locale }) {
  ...
  return (
    <header className="site-header" ref={headerRef}>
      ...
      <div
        id="mobile-menu"
        className={`overflow-hidden border-t border-[rgba(244,235,218,0.25)] bg-[var(--terracotta)] transition-[max-height,opacity] duration-300 ease-in-out md:hidden ${
      ...
      >
        <nav className="flex flex-col gap-1 px-6 py-4">
          {NAV.map((item) => (
            <Link
              ...
              className={`rounded-lg px-2 py-3 text-base transition-colors hover:bg-[rgba(244,235,218,0.12)] hover:text-[var(--cream-on-dark)] ${
                item.active ? "text-[var(--cream-on-dark)] font-semibold" : "text-[var(--cream-on-dark)] opacity-80"
              }`}
            >
              {item.label}
            </Link>
        ...
        <div className="flex items-center justify-between gap-4 border-t border-[rgba(244,235,218,0.25)] px-6 py-4">
          <div className="flex items-center gap-1.5 text-xs uppercase tracking-wider text-[var(--cream-on-dark)] opacity-80">
            {LOCALES.map((l, i) => (
              <span key={l} className="flex items-center gap-1.5">
                ...
                <Link
                  href={localizePath(pathname, l)}
                  onClick={() => setOpen(false)}
                  className={l === locale ? "text-[var(--cream-on-dark)] opacity-100" : "transition-opacity hover:opacity-100"}
                >
                  {l.toUpperCase()}
                </Link>
              </span>
          ...
          <a
            ...
            aria-label={t.nav.whatsappLabel}
            onClick={() => setOpen(false)}
            className="flex h-9 w-9 items-center justify-center text-[var(--cream-on-dark)] transition-opacity hover:opacity-70"
          >
